...
# 提取name當中的title，與社會地位與年齡有關
train_df['Title'] = train_df['Name'].str.extract(' ([A-Za-z]+)\\.', expand=False)
test_df['Title'] = test_df['Name'].str.extract(' ([A-Za-z]+)\\.', expand=False)

# 簡化 Title，分類罕見頭銜為 'Rare'
...
# 確保 test_df 有與 train_df 相同的 Title 欄位（若 test 缺少某 Title，補 0）
...
# 保持欄位一致（排除非特徵欄位）
test_df = test_df[train_df.columns.drop(['Survived', 'Name', 'Ticket', 'Cabin'])]

# 特徵
features = ['Pclass', 'Sex', 'Age', 'SibSp', 'Parch', 'Fare', 'Embarked'] + [col for col in train_df.columns if col.startswith('Title_')]
X = train_df[features]
y = train_df['Survived']
